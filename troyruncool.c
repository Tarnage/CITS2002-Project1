...
// Convert hex string from read function to integer
int charToDecimal(char* a[], int size){
    int i;
    
    for(i = 0;  i < a[size]!='\0'; i = i + 2, size = size -2){
        // the first element of the hex string
        int P = -1;
        int x;
        int y;
        ...
        // Scond element of the hex string
        if(a[i+1] >= '0' && a[i+1] <= '9')
            x = (a[i+1] - '0') * 16 *16;
